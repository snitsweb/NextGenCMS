...
class Application {
  ...
  readyToAddSections() {
    ...
    this.#sections.push(
      new Section("social_media_section", SocialMediaSection)
    );
  }
  ...
  setReactRouterRoutes() {
    this.#reactRouterRoutes = this.#db.subpages
      .map((page) => {
        ...
        return {
          ...
          element: (
            <Layout>
              <Page sections={this.getSections(page.meta.path)} />
            </Layout>
          ),
        };
      })
  ...
  getSections(pagePath) {
    return this.#db.subpages
      .find((page) => page.meta.path === pagePath)
      .value.sections?.map((section) => {
        return {
          value: section,
          component: this.#sections.find(
            (entitySection) => entitySection.alias === section.alias
          )?.component,
        };
      });
  }
  ...
  setLayout() {
    const layout =
      this.#layouts.find((layout) => layout.alias === this.#db.layout?.alias) ||
